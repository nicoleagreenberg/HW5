...
#DONE # TODO 364: Create a database in postgresql in the code line below, and fill in your app's database URI. It should be of the format: postgresql://localhost/YOUR_DATABASE_NAME

## Your final Postgres database should be your uniqname, plus HW5, e.g. "jczettaHW5" or "maupandeHW5"
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://nicoleackermangreenberg@localhost:5432/nicoleagHW5"
## Provided:
...
# DONE # TODO 364: Define an UpdateButtonForm class for use to update todo items
class UpdateButtonForm(FlaskForm):
    submit= SubmitField("Update this to do item")


# DONE# TODO 364: Define a form class for updating the priority of a todolist item
#(HINT: What class activity you have done before is this similar to?)
class UpdatePriorityForm(FlaskForm):
    new_priority = StringField("What is the new priority of this to do list item",  validators=[Required()])
    submit= SubmitField("Update priority of this item")


# DONE # TODO 364: Define a DeleteButtonForm class for use to delete todo items
...
# DONE # TODO 364: Update the all_lists.html template and the all_lists view function such that there is a delete button available for each ToDoList saved.
...
# DONE #TODO 364: Update the one_list view function and the list_tpl.html view file so that there is an Update button next to each todolist item, and the priority integer of that item can be updated. (This is also addressed in later TODOs.)
...
# DONE #TODO 364: Complete route to update an individual ToDo item's priority
@app.route('/update/<item>',methods=["GET","POST"])
def update(item):
    form = UpdatePriorityForm()
    if form.validate_on_submit():
        updated_priority = form.new_priority.data
        item = TodoItem.query.filter_by(description = item).first()
        item.priority = updated_priority
        db.session.commit()
        flash("The priority of " + item.description + " has been successfuly updated")
        return redirect(url_for('all_lists'))
...
# DONE# TODO 364: Fill in the update_item.html template to work properly with this update route. (HINT: Compare against example!)

# TODO 364: Complete route to delete a whole ToDoList
@app.route('/delete/<lst>',methods=["GET","POST"])
def delete(lst):
    this_list = TodoList.query.filter_by(id=lst).first()
    db.session.delete(this_list)
    db.session.commit()
    flash("List " + this_list.title + " has been successfully deleted")
    return redirect(url_for("all_lists"))
